...
def get_data(path, split, num):                   #num为数据集中样本的个数
    ...
    if os.path.exists('X_Train111.npy') and os.path.exists('X_Test111.npy'):
        print("READING data")
        if split == 'train/data':
...
###################模型训练#########
def train(x, y, net, Loss, optimizer,Z,pat,N):
    ...
    for t in range(30):
        ...
        print("第{}类数据第{}次, MSEloss为 {}".format(N+1, t + 1, loss.item()))
...
###################模型测试#########
def test(x, y, net_path, Loss, Z, N, LOSS):
    y_pred = net(x.float())
    y_pred = y_pred.float()
    for j in range(51):
        for i in range(2):
    ...
    N=int(N)
    LOSS[0,N] = loss.item()
    print("测试完成, 第{}类数据MSEloss为 {}".format(N+1, loss.item()))
    return 0


if __name__ == '__main__':
    start = datetime.datetime.now()
    path = 'D:/Git/Gitworkspace/脊柱疾病智能诊断/'  #在不同机器上运行时，将path改成相应地址即可
    split = 'train/data'
    num = 149    #训练集样本数
    ...
    LOSS=np.zeros((1,11))
    for I in range(0,22,2):  #对11类脊椎部位分别进行预测
        Y_Train = np.zeros((149,2))
        Y_Test = np.zeros((51, 2))
        Y_Train[:, 0] = Y_train[:, I]
        Y_Train[:, 1] = Y_train[:, I + 1]
        Y_Test[:, 0] = Y_test[:, I]
        Y_Test[:, 1] = Y_test[:, I + 1]
        ...
        Y_Test = Y_Test.float()
        Net1 = train(X_train, Y_Train, net, Loss, optimizer,Z_train,path,I/2)#进行训练
        test(X_test , Y_Test, Net1, Loss, Z_test,I/2,LOSS)#进行测试
    print("TOTAL LOSS：")#输出11类坐标的MSE误差平均值
    L=0
    for i in range(0,11):
        i=int(i)
        L=L+LOSS[0,i]
